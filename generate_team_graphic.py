...
def fetch_item_id(item_name):
    ...
    csv_file = BASE_PATH / "assets/data/items.csv"
    try:
...
def get_pokedex_data(pokemon_name):
    ...
    csv_file = BASE_PATH / "assets/data/pokemon.csv"
    try:
        with open(csv_file, mode='r', encoding='utf-8') as file:
            reader = csv.DictReader(file)
            for row in reader:
...
def get_sprite_path(pokedex_number):
    ...
    if "_" in pokedex_number:
        ...
        return BASE_PATH / f"assets/sprites/{base_number.zfill(4)}_{suffix}.png"
    else:
        return BASE_PATH / f"assets/sprites/{pokedex_number.zfill(4)}.png"
...
def create_pokemon_graphic(pokemon, image, scale=2.0):
    ...
    font_bold = ImageFont.truetype(str(BASE_PATH / "assets/fonts/Roboto-Bold.ttf"), int(28 * scale))  # Increased size
    font_medium = ImageFont.truetype(str(BASE_PATH / "assets/fonts/Roboto-Medium.ttf"), int(24 * scale))  # Increased size
    ...
    if pokemon.gender:
        gender_icon_path = BASE_PATH / f"assets/genders/{'male' if pokemon.gender == 'M' else 'female'}.png"
        try:
    ...
    item_icon_path = BASE_PATH / f"assets/items/item_{item_id}.png"
    try:
    ...
    for type_name in types:
        type_icon_path = BASE_PATH / f"assets/types/{type_name.lower()}.png"
        try:
    ...
    if len(types) == 1:
        none_icon_path = BASE_PATH / "assets/types/none.png"
        try:
    ...
    tera_icon_path = BASE_PATH / f"assets/types/{pokemon.tera_type.lower()}.png"
    try:
    ...
    for move, move_type in move_types:
        type_icon_path = BASE_PATH / f"assets/types/{move_type.lower()}.png"
        try:
            # Load the PNG type icon
...
def create_teamsheet(team, output_path):
    ...
    for i, pokemon in enumerate(team):
        ...
        number_font = ImageFont.truetype(str(BASE_PATH / "assets/fonts/Roboto-ExtraBoldItalic.ttf"), int(60 * 0.85))  # Slightly larger font size
